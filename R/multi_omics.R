...
PlotMultiDensity <- function(imgNm, dpi=72, format="png",factor="1", interactive=F){
  ...
  type<-merged.df$type
  merged.df$Dataset <- factor(merged.df$Dataset, levels = unique(merged.df$Dataset))
  g <- ggplot(merged.df, aes(x=values, group=Dataset)) + 
      geom_line(aes(color=Dataset), stat="density", alpha=0.1) + 
